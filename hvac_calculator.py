...
def generate_pdf_report(data: dict, result: dict, mode: str) -> str:
    """Ultra simple PDF - no emojis, no special characters"""
    pdf = FPDF()
    pdf.add_page()
    ...
    if "Heating" in mode:
        pdf.cell(0, 6, f"Total Heating Load: {result.get('total_btu_hr')} BTU/hr", ln=True)
        pdf.cell(0, 6, f"Airflow: {result.get('cfm')} CFM", ln=True)
    else:
        pdf.cell(0, 6, f"Total Cooling Load: {result.get('total_btu_hr')} BTU/hr", ln=True)
        pdf.cell(0, 6, f"Tons: {result.get('tons')} Tons", ln=True)
        pdf.cell(0, 6, f"Airflow: {result.get('cfm')} CFM", ln=True)
